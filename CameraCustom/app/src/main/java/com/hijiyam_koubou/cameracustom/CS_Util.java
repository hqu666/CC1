...
//							}
	...
	public String timestampDisp(long timestamp){
		String retStr="";
		long tStamp = timestamp / 1000;
		long _elapsedRealtime = elapsedRealtime();
		long currentTimeMillis = System.currentTimeMillis();
		long currentSysUpTime = currentTimeMillis - _elapsedRealtime;
		tStamp = currentSysUpTime + tStamp;
		final DateFormat df = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
		retStr =  df.format(tStamp);            //long ;APIL17;この結果が最後に確認されたときのタイムスタンプ（ブート以降）。
		return    retStr;
	};
